...
<template>
  <transition name="fade">
    <div
      v-if="visible"
      class="fixed w-[300px] overflow-hidden bottom-0 right-0 m-4 bg-red-900 dark:bg-red-950/80 border-red-700 border-1 text-white rounded-lg shadow-md"
    >
      <div class="flex items-center justify-between px-4 py-3">
        <span>{{ message }}</span>
        <button @click.stop="closeToast" class="text-xs font-bold">X</button>
      </div>
      <div class="w-full bg-red-700 rounded-full h-1 mb-[0.4px]">
        <div class="w-0 bg-red-500 h-1 rounded-full" :style="{ width: percent + '%' }"></div>
      </div>
    </div>
  </transition>
</template>
